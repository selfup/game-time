...
const logNames = () => {
  $('#start').on('click', (event) => {
    playerOneName = window.location.href.split('?firstname=').join("").split('&lastname=')[0].replace('http://localhost:8080/', '')
    playerTwoName = window.location.href.split('?firstname=').join("").split('&lastname=')[1].replace('http://localhost:8080/', '')
