...
const logNames = () => {
  $('#start').on('click', (event) => {
    playerOneName = window.location.href.split('?playerone=').join("").split('&playertwo=')[0].replace('http://localhost:8080/', '')
    playerTwoName = window.location.href.split('?playerone=').join("").split('&playertwo=')[1].replace('http://localhost:8080/', '')
